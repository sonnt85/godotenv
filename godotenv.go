...
// Parse reads an env file from io.Reader, returning a map of keys and values.
func LoadStdoutCmd(cmd string, cmdArgs ...string) (err error) {
...
// Parse reads an env file from io.Reader, returning a map of keys and values.
func LoadStdoutScript(script string, overloads ...bool) (err error) {
	var envMap map[string]string
	envMap, err = ParseStdoutScript(script)
	if err != nil {
		return err
	}
	overload := true
...
// Parse reads an env file from io.Reader, returning a map of keys and values.
func ExeEnvFromStdoutScript(script string, cmd string, cmdArgs ...string) (err error) {
	var envMap map[string]string
	envMap, err = ParseStdoutScript(script)
	if err != nil {
		return err
...
func ParseStdoutScript(script string) (envMap map[string]string, err error) {
	shellbin := ""
	...
	if len(shellbin) == 0 {
		err = errors.New("missing binary shell")
		return
	}
	command := exec.Command(shellbin)
	command.Stdin = bytes.NewBuffer([]byte(script))
	var stdout io.ReadCloser
	stdout, err = command.StdoutPipe()
	if err != nil {
		return
