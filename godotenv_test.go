...
var printenvfiletemplateEmbed string = `
set -a
. %s
__keys=$(
    sh <<EOF
...
for key in ${__keys}; do
    eval val="\$$key"
    val=$(echo "$val" | sed 's/"//g')
    # val=${val//\"/}
    if [ "$key" != "_" ] && [ "$key" != SHLVL ]; then
        printf '%%s="%%s"\n' "${key}" "${val}"
    fi
done
`
...
func TestEnvScripts(t *testing.T) {
	b := `# SERVERROOT=/var/www/html/nextcloud
: ${NEXTCLOUD_MYSQL_DATABASE:=nextcloud}
: ${NEXTCLOUD_MYSQL_USER:=nextclouduser}
: ${NEXTCLOUD_MYSQL_PASSWORD:=nc123}
: ${NEXTCLOUD_MYSQL_HOST:=localhost:/run/mysqld/mysqld.sock}
: ${NEXTCLOUD_ADMIN_USER:=admin}
: ${NEXTCLOUD_ADMIN_PASSWORD:=admin}
: ${NEXTCLOUD_TRUSTED_DOMAINS:="ecloud.iotsvn.com ecloud.ehomevn.com ecloud.$(hostname)" ${LANIP}}
: ${NEXTCLOUD_UPDATE:=1}
: ${NEXTCLOUD_INIT_LOCK:=true}
: ${NEXTCLOUD_INIT_HTACCESS:=true}
: ${REDIS_HOST:=/var/run/redis/redis.sock}
: ${REDIS_HOST_PASSWORD:=adminpwd}
: ${REDIS_HOST_PORT:=0}
: ${NEXCLOUD_SERVERROOT:=/var/www/nextcloud}
: ${NEXTCLOUD_DATA_DIR:=${NEXCLOUD_SERVERROOT}/data}
: ${NEXTCLOUD_ONLYDOC:="https://onlydoc.${DEFAULT_DOMAIN}/"}
: ${NEXTCLOUD_LIBREOFFICE:="https://libreoffice.${DEFAULT_DOMAIN}/"}
: ${NEXTCLOUD_ONLYDOC_JWT_SECRET:=tuadenz}
: ${NEXTCLOUD_STURN_SERVERS:=turn.${DEFAULT_DOMAIN}}
: ${NEXTCLOUD_TURN_SERVERS:=turn.${DEFAULT_DOMAIN}}
